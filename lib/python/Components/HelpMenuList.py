			...
			for (action, help) in actions:
				...
				if isinstance(help, list):
					...
					if config.skin.xres.value == 1920:
						entry.extend((
							(eListboxPythonMultiContent.TYPE_TEXT, 0, 0, 600, 40, 0, 0, help[0]),
							(eListboxPythonMultiContent.TYPE_TEXT, 0, 42, 600, 30, 1, 0, help[1])
						))
					else:
						entry.extend((
					...
					if config.skin.xres.value == 1920:
						entry.append( (eListboxPythonMultiContent.TYPE_TEXT, 0, 0, 600, 42, 0, 0, help) )
					else:
