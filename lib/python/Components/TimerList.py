...
#
	def buildTimerEntry(self, timer, processed):
		...
		if config.skin.xres.value == 1920:
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 0, width, 45, 0, RT_HALIGN_LEFT|RT_VALIGN_CENTER, timer.service_ref.getServiceName()))
			res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 45, width, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, timer.name))
		else:
		...
		if timer.repeated:
			...
			if timer.justplay:
				if timer.end - timer.begin < 4: # rounding differences
					if config.skin.xres.value == 1920:
						res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + ((" %s "+ _("(ZAP)")) % (FuzzyTime(timer.begin)[1]))))
					else:
					...
					if config.skin.xres.value == 1920:
						res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + ((" %s ... %s (%d " + _("mins") + ") ") % (FuzzyTime(timer.begin)[1], FuzzyTime(timer.end)[1], (timer.end - timer.begin) / 60)) + _("(ZAP)")))
					else:
				...
				if config.skin.xres.value == 1920:
					res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + ((" %s ... %s (%d " + _("mins") + ")") % (FuzzyTime(timer.begin)[1], FuzzyTime(timer.end)[1], (timer.end - timer.begin) / 60))))
				else:
			...
			if timer.justplay:
				if timer.end - timer.begin < 4:
					if config.skin.xres.value == 1920:
						res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + (("%s, %s " + _("(ZAP)")) % (FuzzyTime(timer.begin)))))
					else:
					...
					if config.skin.xres.value == 1920:
						res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + (("%s, %s ... %s (%d " + _("mins") + ") ") % (FuzzyTime(timer.begin) + FuzzyTime(timer.end)[1:] + ((timer.end - timer.begin) / 60,))) + _("(ZAP)")))
					else:
				...
				if config.skin.xres.value == 1920:
					res.append((eListboxPythonMultiContent.TYPE_TEXT, 0, 75, width-225, 30, 1, RT_HALIGN_LEFT|RT_VALIGN_CENTER, repeatedtext + (("%s, %s ... %s (%d " + _("mins") + ")") % (FuzzyTime(timer.begin) + FuzzyTime(timer.end)[1:] + ((timer.end - timer.begin) / 60,)))))
				else:
		...
		if config.skin.xres.value == 1920:
			res.append((eListboxPythonMultiContent.TYPE_TEXT, width-225, 75, 225, 30, 1, RT_HALIGN_RIGHT|RT_VALIGN_CENTER, state))
		else:
